...
; === Setup PORTD for switches ===
    ...
    MOVWF TRISD ; set ((NVMCON1) and 0FFh), 0, b[1:0] as inputs
...
CHECK_SWITCHES:
    ...
    BTFSC PORTD, 0 ; ((PORTA) and 0FFh), 0, a (Switch A - Increment) ; skip next instruction if Switch A is not pressed
    GOTO CHECK_B_WHILE_A ; if switch A is pressed, check switch B next

    ; if switch A is NOT pressed, check switch B
    BTFSC PORTD, 1 ; ((PORTA) and 0FFh), 1, a (Switch B - Decrement) ; skip next instruction if switch B is not pressed
...
CHECK_B_WHILE_A:
    ...
    BTFSS PORTD, 1 ; ((PORTA) and 0FFh), 1, a (Switch B - Decrement) ; skip next instruction if switch B is also pressed
