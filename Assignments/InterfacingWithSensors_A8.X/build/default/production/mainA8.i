...
_Bool check_code(uint8_t user_code);
void activate_motor(void);
void activate_buzzer(void);
void emergency_melody(void);
void __attribute__((picinterrupt(("irq(8), base(0x400)")))) INT0_ISR(void);
...
uint8_t input_digit(uint8_t sensor_pin) {
    uint8_t count = 1;
    while (PORTEbits.RE0 == 1 || PORTEbits.RE1 == 1);
    display_digit((char)(count + '0'));

    while (1) {

        if (sensor_pin == 0 && PORTEbits.RE0 == 1 && count < 4) {
        ...
        else if (sensor_pin == 1 && PORTEbits.RE1 == 1 && count < 4) {
            _delay((unsigned long)((500)*(4000000/4000.0)));
            count++;
...
void check_for_PR_input(void) {
    if(PORTEbits.RE0 == 1 || PORTEbits.RE1 == 1){
        if (PORTEbits.RE0 == 1){
            high_digit = input_digit(0);

            _delay((unsigned long)((500)*(4000000/4000.0)));
        }else if (PORTEbits.RE1 == 1){
            low_digit = input_digit(1);
...
uint8_t set_new_secret_code(void) {
    ...
    uint8_t high = wait_for_keypad_digit();
    display_digit((char)(high + '0'));
    _delay((unsigned long)((1000)*(4000000/4000.0)));

    uint8_t low = wait_for_keypad_digit();
